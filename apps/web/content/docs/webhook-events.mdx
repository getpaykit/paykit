...
description: How PayKit handles Stripe webhooks and fires application events.
---

PayKit handles Stripe webhooks automatically. It verifies signatures, normalizes events, syncs local billing state, and then fires your application handlers.
...
Use `paykitjs listen` to receive Stripe webhooks while your app runs locally:
...
PayKit records each incoming webhook event and processes it idempotently. If Stripe sends the same event more than once, PayKit skips the duplicate. Your `on` handlers won't fire twice for the same event.

...
<Steps>
  <Step>
    <StepTitle>Define plans</StepTitle>
    ...
    Pass your products to `createPayKit`:
  ...
  <Step>
    <StepTitle>Create a customer</StepTitle>
  ...
  <Step>
    <StepTitle>Subscribe to a paid plan</StepTitle>
    ...
    Don't treat the subscription as active yet if `paymentUrl` is set. It activates after Stripe confirms payment via webhook.
  </Step>

  <Step>
    <StepTitle>Handle the webhook</StepTitle>

    After checkout, Stripe sends a webhook to PayKit's endpoint. PayKit verifies the signature, syncs the subscription locally, and fires a `customer.updated` event. This is fully automatic.
  ...
  <Step>
    <StepTitle>Check entitlements</StepTitle>
  ...
  <Step>
    ...
    Any prorated amount is handled by Stripe based on your billing settings.
  </Step>

  <Step>
    <StepTitle>Downgrade</StepTitle>
  ...
  <Step>
    <StepTitle>Listen to changes</StepTitle>
